...
namespace ImGuiSDL
{
	void Initialize(SDL_Renderer* renderer, int windowWidth, int windowHeight)
	{
		...
		io.Fonts->TexID = (unsigned int)texture;
					...
					for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3)
					{
						...
						if (i + 6 <= drawCommand->ElemCount)
						{
							...
							&& bounding.IsOnExtreme(v5.pos))
							{
								...
								if (isWrappedTexture)
								{
									DrawRectangle(bounding, (Texture*)(drawCommand->TextureId), Color(v0.col), doHorizontalFlip, doVerticalFlip);
								}
								else
								{
									DrawRectangle(bounding, (SDL_Texture*)(drawCommand->TextureId), Color(v0.col), doHorizontalFlip, doVerticalFlip);
								}
						...
						else
						{
							...
							DrawTriangle(v0, v1, v2, (const Texture*)(drawCommand->TextureId));
						}
					}
				}
