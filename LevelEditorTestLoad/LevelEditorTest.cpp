...
void LoadGame()
{
	...
	level = new Level(reader, "Level1.bin");
	auto vecGO = level->GetGrid()->GetGameObjects();
	auto vecEnemies = level->GetEnemies();
	for (size_t i{}; i < vecGO.size(); ++i)
	{
		(*pScene).Add(vecGO[i]);
	}
	for (size_t i{}; i < vecEnemies.size(); ++i)
	{
		(*pScene).Add(vecEnemies[i]);
	}
}
...
void SaveFile()
{
	...
	//std::cin >> name;
	//grid->WriteToFile(writer, name.c_str());
...
int main(int, char* [])
{
	...
	LoadGame();
	{
		...
		while (doContinue)
		{
			...
			//std::cout << "Current index is " << RectIndex << '\n';
